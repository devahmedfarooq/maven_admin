...
const fetchBlog = async (id: string) => {
  const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/blogs/${id}`);
  return data;
};

const updateBlog = async ({ id, values }: { id: string; values: BlogData }) => {
  const { data } = await axios.put(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/blogs/${id}`, values);
  return data;
};

const deleteBlog = async (id: string) => {
  await axios.delete(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/blogs/${id}`);
};
