...
const fetchBlogs = async () => {
  const { data } = await axios.get( process.env.NEXT_PUBLIC_API_URL + "/blogs");
  return data;
};

const createBlog = async (blogData: BlogData) => {
  const { data } = await axios.post(process.env.NEXT_PUBLIC_API_URL + "/blogs", blogData);
  return data;
};
...
const uploadImage = async (file: File) => {
  ...
  const { data } = await axios.post(process.env.NEXT_PUBLIC_API_URL + "/utils/image", formData, {
